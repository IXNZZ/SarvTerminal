...
**Detection state machine** (`SSHConnectionController`, main-run-loop `Timer` at **two rates**: 0.2s while handshaking, 2.0s once connected -- see below). There is no `ssh` API to observe — success/failure is inferred by **polling the surface's visible text** (`SurfaceView.liveVisibleText()`) and its `childExitedMessage`:
...
- Once `connected`, keeps polling for the session ending → `.disconnected` then `scheduleReconnect()`.
- **Drop the poll rate once connected, and give every repeating timer a tolerance.** The handshake poll must be fast (0.2s): it scrapes visible terminal text for prompts and failures while the user watches a progress card, so latency is visible. The *connected* watch is a different job — it only reads two booleans (`sessionEnded()`), and the recovery it starts counts down in whole seconds, so a fast rate buys nothing. It costs a lot, though, because this timer **outlives the card and lives as long as the tab**: at 5Hz a wall of connected tabs is tens of main-loop wakeups per second competing with keystrokes and rendering, and users read that as "the terminal is slower than the system one". Run the connected watch at 2.0s, and set a slack/tolerance of ~50% on every repeating timer so the platform can coalesce them into shared wakeups instead of each tab forcing its own. Detection latency becomes up to ~3s, which is invisible next to the reconnect back-off.
- **The end-of-session signal must be the CORE's process state, never a UI message.** `sessionEnded()` is `childExitedMessage != nil || processExited`, and `processExited` (`ghostty_surface_process_exited`, i.e. the core's `child_exited` flag) is the authoritative half: it is true the moment `ssh` dies regardless of whether the tab is on screen. `childExitedMessage` is UI-dependent — the apprt only accepts the core's `show_child_exited` action when the surface is **in a window** (`Ghostty.App.showChildExited` guards on `surfaceView.window != nil`), and only the selected tab's pane is in the view hierarchy. For a BACKGROUND tab the action is refused, so the core falls back to writing *"Process exited. Press any key to close the terminal."* into the terminal (`Surface.zig`) and, with `wait_after_command`, waits for a keypress before posting the close. Watching only that message meant a background session that dropped overnight sat on a dead terminal with no card and no auto-reconnect until the user pressed a key — the keypress was what finally posted the close and started the recovery. Any port has the same trap: a per-widget "is it visible" guard on the exit notification is normal, so poll the core flag instead.

**Wake / network-return resume.** Reachability (`NWPathMonitor`) and wake-from-sleep both call `retryReconnectingNow(reason:)`, which asks **every** registered connection — not only those already counting down — via `controller.resumeIfReconnectable(reason:)`. That method reconnects immediately when the connection is `.disconnected`, `.failed` with an `isAutoRetriable` cause, **or still marked `.connected` while its ssh process is already gone**. The last case is required because no timer runs while the machine sleeps, so an overnight drop is still unnoticed at wake time and would never enter the reconnect loop; `resumeIfReconnectable` force-detects it (`handleProcessExited()` then skip the countdown). States that need the USER are never touched: `.needsPassword`, `.needsHostKey`, and non-retriable failures (`permissionDenied`). The card's **Stop** button sets `model.autoReconnectStopped`, which makes `resumeIfReconnectable` a no-op — otherwise a wake event would reconnect a session the user explicitly told us to leave alone; any deliberate reconnect and any successful connect clear the flag.
...
| `Timer` + `RunLoop.main` (0.2s handshake poll, 2.0s connected watch, 1s countdown) | Detection loop, reconnect countdown | `g_timeout_add_full` on the GLib main loop; use `g_timeout_add_seconds` for the 2.0s connected watch — its one-second granularity lets GLib align the wakeup with other timers, which is exactly the coalescing `Timer.tolerance` buys on macOS. |
...
| `SurfaceView.liveVisibleText()` / `ghostty_surface_process_exited` | Polling for prompt/error/exit | Same libghostty core surface data — expose the equivalent readers to the GTK apprt (`src/apprt/gtk`). This is core, not AppKit-specific, and `ghostty_surface_process_exited` is already a C API. |
| `SurfaceView.childExitedMessage` (the `show_child_exited` action) | The *cosmetic* exit banner only | GTK's handler will likewise want the widget to be realized/visible. **Do not build session-end detection on it** — see the detection state machine above; use the process-exited flag. |
...
8. **Background-tab drop (the regression this guards):** connect an SSH tab, switch to a DIFFERENT tab, then kill the session server-side. Within a second the background tab must already be reconnecting — switch back and you should see the card counting down or a live session, never *"Process exited. Press any key to close the terminal."* waiting for a keypress.
9. **Sleep/offline recovery:** with several SSH tabs connected, disconnect the network (or suspend the machine) for longer than the back-off cap, then restore it. Every server tab reconnects on its own; a tab whose countdown was stopped with **Stop** stays stopped. Nothing prompts for a password on its own.
10. **Keychain section:** generate an ed25519 key → files appear in `~/.ssh`, list refreshes, "Copy public key" yields the `.pub` contents, delete removes both files.
11. **Askpass env sanity:** `SSH_ASKPASS_REQUIRE=force` is honored by the target distro's OpenSSH (check `ssh -V` ≥ 8.4); if not, the helper won't be invoked and the connect will hang on the TTY prompt — that is the key portability risk to test first.
